...
class dngTag(object):
    ...
    def setValue(self, value):
        if self.DataType == Type.Byte:
            ...
            self.Value = struct.pack('<%sL' % (len(value) * 2), *[item for sublist in value for item in sublist])  # ... This... uhm... flattens the list of two value pairs
...
def process(input_file_name_jpeg, input_file_name_raw, width, height, depth):

    # extract tags
    with open(input_file_name_jpeg,'rb') as fff:
        tags = exifread.process_file(fff)

    # extract bayer RAW to ${DEPTH}bit numpy array
    file = open(input_file_name_raw, 'rb')
    f = np.fromfile(file, dtype={
        8  :np.uint8,
        16 :np.uint16,
        32 :np.uint32
        }[depth], count=height * width)
    rawImage = f.reshape((height, width))
    file.close() 
    ...
    rawImage_ = np.zeros(rawImage.shape)
    rawImage_[0::2, 1::2] = rawImage[1::2, 0::2]
    rawImage_[1::2, 0::2] = rawImage[0::2, 1::2]
    rawImage_[0::2, 0::2] = rawImage[1::2, 1::2]
    rawImage_[1::2, 1::2] = rawImage[0::2, 0::2]
...
def convert(input_file_name_jpeg, input_file_name_raw, width, length):
    ...
    rawFrame, tags = process(input_file_name_jpeg, input_file_name_raw, width, length, 16)
...
def main():
    ...
    if len(args) < 1:
        ...
        input_file_name_jpeg = args[0]
        input_file_name_raw = args[1]

    print
    print
    print
    print input_file_name_jpeg, input_file_name_raw, width, length
    convert(input_file_name_jpeg, input_file_name_raw, width, length)
